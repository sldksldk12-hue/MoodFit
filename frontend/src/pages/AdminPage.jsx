...
const UserTable = ({ items, currentUserId, currentUserAccount, onRole }) => items.length ? <div className="admin-table-wrap"><table className="admin-table"><thead><tr><th>회원</th><th>이메일</th><th>가입일</th><th>주문</th><th>리뷰</th><th>권한</th></tr></thead><tbody>{items.map((item) => <tr key={item.id}><td><strong>{item.user_account}</strong>{(item.id === currentUserId || (currentUserAccount && item.user_account === currentUserAccount)) && <span className="me-badge">나</span>}</td><td>{item.email}</td><td>{dateText(item.created_at)}</td><td>{item.order_count}건</td><td>{item.review_count}개</td><td><select disabled={item.id === currentUserId || (currentUserAccount && item.user_account === currentUserAccount)} value={item.admin_role} onChange={(event) => onRole(item, event.target.value)}><option value="USER">일반 회원</option><option value="ADMIN">관리자</option></select></td></tr>)}</tbody></table></div> : <Empty />;
